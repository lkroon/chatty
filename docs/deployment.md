...
- [ ] **P4** — After deploying, **reconnect** the Google account: existing `google_connections` rows hold a refresh token granted for the two read-only scopes only, and Google will refuse a write call with `insufficient permissions`. Visit `/auth/google/connect` (e.g. `https://<host>/auth/google/connect`) while logged in and accept the larger consent screen: that route always sends `prompt=consent`, and its callback upserts the connection with the scopes Google returns, so it re-grants in place — there is no Disconnect button in the UI. The confirm endpoint detects the old grant and says exactly this, so a missed reconnect fails loudly rather than silently.
